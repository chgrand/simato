...
void MainWindow::createMenu()
{
  ...
  app_menu->addSeparator();
  QAction *safety_to_wp_group = 
    new QAction("&Safety zone -> Waypoints Group", this);
...
{
  if(!tryEraseExistingMission()) return;
...
void MainWindow::loadFile(const QString& fileName){
  ...
  if(mission_model->read_json(fileName.toStdString())) {
      ...
      for(auto wp_group: mission_model->wp_groups)
        for(auto point_entry: wp_group.second.waypoints) {
          mission::point3_t point = point_entry.second;
     	    if(point.x > mission_model->map_data.x_max) mission_model->map_data.x_max = point.x;
     	    if(point.x < mission_model->map_data.x_min) mission_model->map_data.x_min = point.x;
     	    if(point.y > mission_model->map_data.y_max) mission_model->map_data.y_max = point.y;
     	    if(point.y < mission_model->map_data.y_min) mission_model->map_data.y_min = point.y;
          std::cout << "(" << point.x <<", "<<point.y<<")   ";
          std::cout << "[" << mission_model->map_data.x_min << ", "
	            << mission_model->map_data.x_max << ", "
	            << mission_model->map_data.y_min << ", "
	            << mission_model->map_data.y_max << "]" << std::endl;
    ...
    QString test_filename = 
	  mission_model->get_home_dir().filePath(QString::fromStdString(mission_model->map_data.image));
...
void MainWindow::waypointsAdd()
{
  ...
  if(inputBox.exec()!=0) {
    ...
    mission_viewer->
    getMapViewer()->actionMode(MapViewer::Add_Waypoints, name);
  }
  else
    std::cout << "Outch !!\n";}
...
void MainWindow::waypointsMove()
{
  ...
  if(inputBox.exec()!=0) {
    ...
    mission_viewer->
      getMapViewer()->actionMode(MapViewer::Move_Waypoints, name);
  }
  else
    std::cout << "Outch !!\n";
}
...
void MainWindow::waypointsDelete()
{
  ...
  if(inputBox.exec()!=0) {
    ...
    mission_viewer->
      getMapViewer()->actionMode(MapViewer::Delete_Waypoints, name);
  }
  else
    std::cout << "Outch !!\n";
}
...
void MainWindow::patrolAdd()
{
  ...
  if(inputBox.exec()!=0) {
    ...
    //mission_viewer->setFilter(name);
    mission_viewer->
      getMapViewer()->actionMode(MapViewer::Patrol_Add, name);
  }
  else
    std::cout << "Outch !!\n";
}
...
// define safety zone
{
  ...
  if(inputBox.exec()!=0) {
    ...
    mission_viewer->
      getMapViewer()->actionMode(MapViewer::Add_Safety_Zone, agent_name);
  }
  else
    std::cout << "Outch !!\n";
}
