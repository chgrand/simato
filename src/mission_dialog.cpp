...
void MissionDialog::selectFile()
{
  ...
  QString globalFileName = 
    QFileDialog::getOpenFileName(this,"Select file", home_path);
  ...
  if(globalFileName!="") {
    ...
    if(button==map_image_btn) map_image_edit->setText(globalFileName);
    if(button==map_region_btn) map_region_edit->setText(globalFileName);
    if(button==map_dtm_btn) map_dtm_edit->setText(globalFileName);
  }
}
...
void MissionDialog::initData(MissionModel *mission_model)
{
  ...
  map_region_edit->
    setText(QString::fromStdString(mission_model->map_data.region_geotiff));
  map_dtm_edit->
    setText(QString::fromStdString(mission_model->map_data.dtm_geotiff));
  map_blender_edit->
    setText(QString::fromStdString(mission_model->map_data.blender));
