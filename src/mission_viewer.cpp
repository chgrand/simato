  ...
  mission_model(model)
{
  ...
  QObject::connect(map_viewer, SIGNAL(mouseNewPosition(QPointF)),
		   this, SLOT(onMouseOverMap(QPointF)));

  QObject::connect(map_view_check, SIGNAL(stateChanged(int)),
		   map_viewer, SLOT(setMapView(int)));

  QObject::connect(grid_view_check, SIGNAL(stateChanged(int)),
		   map_viewer, SLOT(setGridView(int)));

  QObject::connect(alpha_spin, SIGNAL(valueChanged(int)),
		   map_viewer, SLOT(setAlpha(int)));

  QObject::connect(wp_group_filter, SIGNAL(currentIndexChanged(const QString &)),
		   map_viewer, SLOT(setWpGroupFilter(const QString &)));

  QObject::connect(safety_zone, SIGNAL(stateChanged(int)),
		   map_viewer, SLOT(setZonesFilter(int)));

  QObject::connect(observations_filter, SIGNAL(stateChanged(int)),
		   map_viewer, SLOT(setObservationsFilter(int)));
